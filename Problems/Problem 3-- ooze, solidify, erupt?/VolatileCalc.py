...
def VolatileCalc(func,var1,var2):
    ...
    ###########################################################################
        ...
        if error[0] == 0:
            ...
            # Use integer division to ensure steps is an integer
            P_start = P_interval
            steps = 5000 // P_interval  # Integer division makes `steps` an integer
            P_stop = P_interval * steps  # P_stop is now an integer

            # Ensure all range arguments are integers
            for P in range(P_start, P_stop + P_interval, P_interval):
                routine[1] = float(P)  # Convert P to float if required by other calculations
